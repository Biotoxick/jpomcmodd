...
public class NPCARightClickedOnEntityProcedure {
	public static void execute(LevelAccessor world, Entity sourceentity) {
		...
		if ((sourceentity.getCapability(JpomcModVariables.PLAYER_VARIABLES_CAPABILITY, null)
			...
			if (!(JpomcModVariables.DialogueA1).equals("")) {
				if (sourceentity instanceof Player _player && !_player.level.isClientSide())
					_player.displayClientMessage(new TextComponent(JpomcModVariables.DialogueA1), (false));
			}
			new Object() {
				private int ticks = 0;
				...
				private void run() {
					if (!(JpomcModVariables.DialogueA2).equals("")) {
						if (sourceentity instanceof Player _player && !_player.level.isClientSide())
							_player.displayClientMessage(new TextComponent(JpomcModVariables.DialogueA2), (false));
					}
