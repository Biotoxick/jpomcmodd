...
public class NpcDdialogueProcedure {
	public static void execute(LevelAccessor world, Entity sourceentity) {
		...
		if ((sourceentity.getCapability(JpomcModVariables.PLAYER_VARIABLES_CAPABILITY, null)
			...
			if (!(JpomcModVariables.DialogueD1).equals("")) {
				if (sourceentity instanceof Player _player && !_player.level.isClientSide())
					_player.displayClientMessage(new TextComponent(JpomcModVariables.DialogueD1), (false));
			}
			new Object() {
				private int ticks = 0;
				...
				private void run() {
					if (!(JpomcModVariables.DialogueD2).equals("")) {
						if (sourceentity instanceof Player _player && !_player.level.isClientSide())
							_player.displayClientMessage(new TextComponent(JpomcModVariables.DialogueD2), (false));
					}
