...
public class NpcBdialogueProcedure {
	public static void execute(LevelAccessor world, Entity sourceentity) {
		...
		if ((sourceentity.getCapability(JpomcModVariables.PLAYER_VARIABLES_CAPABILITY, null)
			...
			if (!(JpomcModVariables.DialogueB1).equals("")) {
				if (sourceentity instanceof Player _player && !_player.level.isClientSide())
					_player.displayClientMessage(new TextComponent(JpomcModVariables.DialogueB1), (false));
			}
			new Object() {
				private int ticks = 0;
				...
				private void run() {
					if (!(JpomcModVariables.DiaogueB2).equals("")) {
						if (sourceentity instanceof Player _player && !_player.level.isClientSide())
							_player.displayClientMessage(new TextComponent(JpomcModVariables.DiaogueB2), (false));
					}
