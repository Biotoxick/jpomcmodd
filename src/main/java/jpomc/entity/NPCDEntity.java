...
import jpomc.procedures.NpcDdialogueProcedure;
import jpomc.procedures.NPCAEntityIsHurtProcedure;

import jpomc.init.JpomcModEntities;

public class NPCDEntity extends PathfinderMob {
	public NPCDEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(JpomcModEntities.NPCD.get(), world);
	}

	public NPCDEntity(EntityType<NPCDEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		NpcDdialogueProcedure.execute(world, sourceentity);
		return retval;
	}
