...
import jpomc.procedures.NPCARightClickedOnEntityProcedure;
import jpomc.procedures.NPCAEntityIsHurtProcedure;

import jpomc.init.JpomcModEntities;

public class NPCAEntity extends PathfinderMob {
	public NPCAEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(JpomcModEntities.NPCA.get(), world);
	}

	public NPCAEntity(EntityType<NPCAEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
		...
		NPCARightClickedOnEntityProcedure.execute(world, sourceentity);
		return retval;
	}
